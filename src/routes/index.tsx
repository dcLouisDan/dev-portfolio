      ...
      <main className="max-w-[1200px] mx-auto bg-white shadow-2xl md:shadow-none min-h-screen md:min-h-0 border-x border-rule/50 md:border-none px-4 md:px-0">
        ...
        <header className="mb-8 pt-8">
          ...
          <div className="border-b-4 border-double border-black mb-8 flex flex-col md:flex-row justify-center items-center py-2 text-sm font-serif-text italic relative">
            <span className="md:absolute md:left-0 uppercase not-italic font-bold text-xs tracking-wider md:block hidden">Vol. 1</span>
            <span className="text-lg">{masthead.title}</span>
            <span className="md:absolute md:right-0 uppercase not-italic font-bold text-xs tracking-wider md:block hidden">Price: Hiring</span>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
          ...
          <aside className="md:col-span-3 md:border-r border-rule md:pr-6">
            ...
            <div className="space-y-6">
              {Object.entries(skills).map(([category, items]) => (
                <div key={category}>
                  <h3 className="font-bold text-sm uppercase tracking-wider mb-2 border-b border-rule/50 pb-1">{category}</h3>
                  <ul className="text-sm leading-relaxed space-y-1 font-serif-text">
                    {items.map((item) => (
                      <li key={item} className="p-0">• {item}</li>
                    ))}
                  </ul>
                </div>
              ))}
          ...
          <section className="md:col-span-6">
            ...
            <article className="mb-12 border-b border-black pb-8">
              <span className="block text-xs font-bold uppercase tracking-widest text-neutral-500 mb-2">Cover Story</span>
                  ...
                  <article key={project.id} className="pb-6 border-b border-rule last:border-0">
                    <div className="flex flex-col md:flex-row gap-4">
                      <div className="md:w-3/4">
                        ...
                        <h3 className="article-headline text-2xl">{project.title}</h3>
          ...
          <aside className="md:col-span-3 md:border-l border-rule md:pl-6">
            ...
            <div className="mb-8">
              ...
              <ul className="space-y-3 font-serif-text text-sm">
                <li>
                  <span className="block font-bold text-xs uppercase tracking-wide text-neutral-500">Email</span>
                  <a href={`mailto:${contact.email}`} className="hover:underline underline-offset-2">{contact.email}</a>
                </li>
                <li>
                  <span className="block font-bold text-xs uppercase tracking-wide text-neutral-500">GitHub</span>
                  <a href={`https://${contact.github}`} target="_blank" rel="noreferrer" className="hover:underline underline-offset-2">{contact.github}</a>
                </li>
                <li>
                  <span className="block font-bold text-xs uppercase tracking-wide text-neutral-500">LinkedIn</span>
                  <a href={`https://${contact.linkedin}`} target="_blank" rel="noreferrer" className="hover:underline underline-offset-2">{contact.linkedin}</a>
                </li>
              </ul>
            </div>

            <div className="border-t-4 border-black pt-4 mb-8">
              <h3 className="font-serif-display font-bold text-lg leading-tight mb-2">About The Paper</h3>
              <p className="font-serif-text text-xs leading-relaxed text-justify">
                This portfolio is designed with a "content-first" philosophy, stripping away modern web embellishments to focus on layout, typography, and information architecture. Inspired by the golden age of print journalism.
              </p>
            </div>

            <div className="border border-black p-2 text-center bg-neutral-50">
              <span className="block text-[10px] uppercase font-bold text-neutral-400 mb-1">Ad Space</span>
              <p className="font-serif-display font-bold text-sm">Now serving pixel-perfect web experiences.</p>
            </div>
